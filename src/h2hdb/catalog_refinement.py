...
artifact plans, analysis worksets, or revision projections. Those properties
belong to the named transaction writer hooks. The recurring checks here are
read-only and bounded to the closed registries, active channel heads, immutable
publication receipts, and the at-most-16-deep analysis seal chain.
"""
...
_EXPECTED_DATA_SEEDS = (
    ...
    (
        "catalog.digest-domain.contributor-sort-as.v1",
        "canonical_digest_policy",
        "digest_domain",
        "contributor_sort_as_utf8_v1",
    ),
    (
...
def _validate_static_catalog_contract() -> None:
    ...
    actual_stage_registries: dict[str, list[tuple[str, tuple[str, str, str]]]] = {
        "analysis_stage": [],
        "publication_stage": [],
    }
    actual_artifact_registries: dict[
        str, tuple[str, tuple[tuple[str, object], ...]]
    ] = {}
    for value in raw:
        ...
        if relation in actual_stage_registries:
            ...
            actual_stage_registries[relation].append(
                (seed_id, (cells[0][3], cells[1][3], cells[2][3]))
            )
            continue
        if relation in {"artifact_zip_writer_policy", "artifact_storage_codec"}:
            if (
                not cells
                or any(not isinstance(cell, tuple) or len(cell) != 4 for cell in cells)
                or any(not isinstance(cell[0], str) for cell in cells)
                or relation in actual_artifact_registries
            ):
                raise BuiltinSemanticRegistryError(
                    f"generated {relation.replace('_', '-')} seed is malformed"
                )
            actual_artifact_registries[relation] = (
                seed_id,
                tuple((cell[0], cell[3]) for cell in cells),
            )
            continue
    ...
    expected_artifact_registries = {
        "artifact_zip_writer_policy": (
            ...
            (
                ...
                ("dos_time", 0),
                ("unix_mode", 33188),
                ("general_purpose_flags", 2048),
                ("create_system", 3),
                ("archive_name_codec_version", 1),
                ("artifact_name_codec_version", 1),
            ),
        ),
        "artifact_storage_codec": (
            ...
            (
                ...
                ("adapter_id", "managed-filesystem"),
                ("locator_codec_version", 1),
                ("protection_token_codec_version", 1),
            ),
        ),
    }
    if actual_artifact_registries != expected_artifact_registries:
        raise BuiltinSemanticRegistryError(
            "generated artifact registry seeds differ from executable constants"
        )
    expected_analysis_stages = tuple(
        ...
        for name, order, cursor_codec in (
    ...
    for backend in ("sqlite", "mariadb"):
        ...
        expected_shapes = {
            ...
            "publication_identity": (
                ...
                ("publication_key",),
                (("publication_id",), ("gid",), ("artifact_name",)),
            ),
            "prepared_artifact": (
...
def _active_source_contexts(connector: SQLConnector) -> tuple[_SourceContext, ...]:
    rows = connector.fetch_all("""
        SELECT channel, source_revision, generation
        FROM catalog_source_heads
        ORDER BY channel
        LIMIT 2
        """)
    if len(rows) > 1:
    ...
    for head_row in rows:
        ...
        if base_row is None:
            ...
            base_revision = _as_int(
                base_row[0], field="base_source_revision", positive=True
            )
            base_generation = _as_int(
                base_row[1], field="base_source_generation", positive=True
            )
            if generation != base_generation + 1 or revision <= base_revision:
                raise CatalogSemanticValidationError(
                    "source_head does not advance its exact build baseline"
                )
            base_revision_row = _one(
                connector,
                """
                SELECT channel
                FROM catalog_source_revisions
                ...
                """,
                (base_revision,),
                detail="source build baseline revision",
            )
            assert base_revision_row is not None
            if (
                _as_bytes(base_revision_row[0], field="baseline source channel")
                != channel
            ):
                raise CatalogSemanticValidationError(
                    "source build baseline belongs to a different channel"
                )
...
def _base_source_tuple(
    ...
    *,
    ...
    if row is None:
        return None
    return (
        _as_int(row[0], field=f"{detail} revision", positive=True),
        _as_int(row[1], field=f"{detail} generation", positive=True),
    )
...
def _validate_publication_candidate_source(
    ...
    *,
    ...
    candidate_base = _base_source_tuple(
        _one(
            connector,
            """
            SELECT base_source_revision, base_source_generation
            FROM catalog_publication_candidate_base_sources
    ...
    if candidate_base != build_base:
        raise CatalogSemanticValidationError(
            "publication candidate base-source CAS differs from its analysis build"
        )

    if candidate_base is None:
        expected_source_generation = 1
    else:
        base_revision, base_generation = candidate_base
        if source_revision <= base_revision:
            raise CatalogSemanticValidationError(
                "publication source revision does not advance its candidate base-source CAS"
            )
        base_revision_row = _one(
            connector,
            """
            ...
            """,
            (base_revision,),
            detail="publication candidate base source revision",
        )
        assert base_revision_row is not None
        if (
            _as_bytes(base_revision_row[0], field="candidate base source channel")
            != channel
        ):
            raise CatalogSemanticValidationError(
                "publication candidate base source belongs to another channel"
            )
...
def _active_publication_contexts(
    ...
    rows = connector.fetch_all("""
        SELECT channel, revision, generation
        FROM catalog_publication_heads
        ORDER BY channel
        LIMIT 2
        """)
    if len(rows) > 1:
    ...
    for head_row in rows:
        ...
        receipt_row = _one(
            connector,
            """
            SELECT receipt_id, source_revision, reserved_revision, channel,
        ...
        if len(_as_bytes(receipt_row[0], field="publication receipt_id")) != 16:
            raise CatalogSemanticValidationError(
        ...
        reserved_revision = _as_int(
            receipt_row[2], field="publication receipt reserved_revision", positive=True
        )
        receipt_channel = _as_bytes(receipt_row[3], field="publication receipt channel")
        ...
        receipt_publication_count = _as_int(
            receipt_row[6], field="publication receipt publication_count"
        )
        if reserved_revision != revision or receipt_channel != channel:
            raise CatalogSemanticValidationError(
        ...
        candidate_row = _one(
            connector,
            """
            SELECT candidate_id, analysis_id, channel, artifact_policy_id,
                   display_title_policy_id, state
        ...
        candidate_id = _as_bytes(candidate_row[0], field="publication candidate_id")
        analysis_id = _as_bytes(candidate_row[1], field="publication analysis_id")
        if len(candidate_id) != 16 or len(analysis_id) != 16:
            raise CatalogSemanticValidationError(
                "active publication candidate identities are not 16 bytes"
            )
        if (
            _as_bytes(candidate_row[2], field="publication candidate channel")
            != channel
            or _as_int(
                candidate_row[3], field="candidate artifact policy", positive=True
            )
            != artifact_policy_id
            or _as_int(
                candidate_row[4], field="candidate display policy", positive=True
            )
            != display_title_policy_id
            or candidate_row[5] != "PUBLISHED"
        ):
            raise CatalogSemanticValidationError(
        ...
        projection_seal_row = _one(
            ...
            SELECT publication_count, artifact_input_count,
                   prepared_artifact_count, create_count, rebuild_count,
                   delete_count, unchanged_count, new_galleries,
        ...
        projection_scalars = tuple(
            _as_int(value, field=f"publication projection seal scalar {index}")
            for index, value in enumerate(projection_seal_row)
        )
        if projection_scalars[0] != publication_count:
            raise CatalogSemanticValidationError(
                "publication projection seal and catalog revision publication_count differ"
            )
        if projection_scalars[7:11] != receipt_result_scalars:
            raise CatalogSemanticValidationError(
                "publication projection seal and receipt result scalars differ"
            )
        if projection_scalars[2] != projection_scalars[3] + projection_scalars[4]:
            raise CatalogSemanticValidationError(
                "publication projection seal prepared-artifact count differs from CREATE plus REBUILD"
            )
        if receipt_state == "PROJECTION_FINALIZED":
            final_checkpoint = _one(
                ...
                LIMIT 2
                """,
                (candidate_id, b"FINALIZE_ARTIFACTS"),
                detail="finalized artifact checkpoint",
            )
            assert final_checkpoint is not None
            checkpoint_generation = _as_int(
                final_checkpoint[0],
                field="finalization checkpoint generation",
                positive=True,
            )
            checkpoint_cursor = _as_bytes(
                final_checkpoint[1], field="finalization checkpoint cursor"
            )
            checkpoint_processed_count = _as_int(
                final_checkpoint[2], field="finalization checkpoint processed_count"
            )
            if final_checkpoint[3] != "COMPLETE" or (
                checkpoint_processed_count != projection_scalars[2]
            ):
                raise CatalogSemanticValidationError(
                    "PROJECTION_FINALIZED lacks its complete projection-count checkpoint"
                )
            terminal_receipt = _one(
                connector,
                """
                ...
                detail="finalized artifact terminal receipt",
            )
            assert terminal_receipt is not None
            if (
                _as_bytes(terminal_receipt[0], field="terminal start_cursor")
                != checkpoint_cursor
                or _as_int(terminal_receipt[1], field="terminal start_processed_count")
                != checkpoint_processed_count
                or _as_bytes(terminal_receipt[2], field="terminal next_cursor")
                != checkpoint_cursor
                or _as_int(terminal_receipt[3], field="terminal next_processed_count")
                != checkpoint_processed_count
                or terminal_receipt[4] != "COMPLETE"
                or _as_int(terminal_receipt[5], field="terminal row_count") != 0
                or _as_int(terminal_receipt[6], field="terminal flag") != 1
                or _as_int(
                    terminal_receipt[7],
                    field="terminal committed_generation",
                    positive=True,
                )
                != checkpoint_generation
            ):
                raise CatalogSemanticValidationError(
                    "PROJECTION_FINALIZED lacks an exact empty terminal batch receipt"
                )
        ...
        base_catalog_row = _one(
            connector,
            """
            SELECT base_revision, base_catalog_generation
            FROM catalog_publication_candidate_base_catalog
            WHERE candidate_id = %s
            LIMIT 2
            """,
            (candidate_id,),
            detail="publication candidate catalog baseline",
            optional=True,
        )
        if base_catalog_row is None:
            if generation != 1:
                raise CatalogSemanticValidationError(
            ...
            base_revision = _as_int(
                base_catalog_row[0], field="base catalog revision", positive=True
            )
            base_generation = _as_int(
                base_catalog_row[1], field="base catalog generation", positive=True
            )
            if generation != base_generation + 1 or revision <= base_revision:
                raise CatalogSemanticValidationError(
        ...
        producer_fields = tuple(
            ...
            for value in artifact_policy_row[4:9]
        )
        ...
        display_policy_row = _one(
            ...
            SELECT display.display_title_algorithm_version,
                   sort.title_sort_algorithm_version,
                   sort.unicode_data_version
            FROM catalog_display_title_policies AS display
            JOIN catalog_title_sort_policy AS sort
              ON sort.title_sort_policy_id = display.title_sort_policy_id
        ...
        unicode_data_version_raw = display_policy_row[2]
...
def check_incremental_impact_v1(connector: SQLConnector) -> None:
    """Require exact active analysis completion seals without scanning worksets."""

    _validate_exact_registries(connector)
    analysis_ids = {
        context.analysis_id for context in _active_source_contexts(connector)
    }
    analysis_ids.update(
        context.analysis_id for context in _active_publication_contexts(connector)
    )
...
def _physical_data_tables() -> Mapping[str, str]:
    ...
    for backend in ("sqlite", "mariadb"):
        ...
        for value in relations:
            if not isinstance(value, Mapping) or value.get("plane") != "data":
                continue
            relation = value.get("relation")
