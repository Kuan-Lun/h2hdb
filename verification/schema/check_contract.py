...
def _validate_transition_authority_contract(
    ...
    ready_terms = (
        "full SchemaAdmin.check scans only the retained publication window",
        "one exact contiguous node/successor chain ending at the maximum commit",
        "successor equals predecessor plus one",
        "no fork/gap/node beyond the tip",
        "every commit generation is present",
        "the common head is the unique maximum tip",
        "every active baseline/protection reference resolves",
        "conservatively include an unreferenced prefix below the oldest commit",
        "later maintenance removes it",
...
def _validate_publication_commit_contract(
    ...
    ready_terms = (
        ...
        "one contiguous successor chain ending exactly at the maximum retained commit",
        ...
        "never a gap, fork, node above the tip, or missing commit generation",
        ...
        "generation zero is required while generation one remains the oldest commit",
        "oldest retained positive node has no incoming edge",
